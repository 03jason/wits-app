...
return function (App $app) {
    ...
    $app->delete('/api/products/{id}', function (Request $req, Response $res, array $args) {
        /** @var \PDO $pdo */
        $pdo = $this->get(\PDO::class);
        $pid = (int)$args['id'];

        $stmt = $pdo->prepare("UPDATE products SET product_available = 0, updated_at = NOW() WHERE product_id = :id");
        $stmt->execute([':id' => $pid]);
        ...
        $res->getBody()->write(json_encode(['status'=>'Product deactivated','product_id'=>$pid], JSON_UNESCAPED_UNICODE));
        return $res->withHeader('Content-Type','application/json');
    });
