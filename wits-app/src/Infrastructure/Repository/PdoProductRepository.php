...
final class PdoProductRepository implements ProductRepositoryInterface
{
    ...
    public function getAllActive(): array {
        $sql = "SELECT product_id, product_name, product_quantity, product_category, created_at, updated_at
                FROM products
    ...
    public function create(array $data): int {
        ...
        if ($name === '' || $qty < 0) { throw new \RuntimeException('Invalid payload'); }

        $this->pdo->beginTransaction();
        try {
            $ins = $this->pdo->prepare("INSERT INTO products
                (product_name, product_quantity, product_category, product_available, created_at, updated_at)
                VALUES (:n,:q,:c,1, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)");
            $ins->execute([':n'=>$name, ':q'=>$qty, ':c'=>$cat]);
            $id = (int)$this->pdo->lastInsertId();

            if ($qty > 0) {
                $mov = $this->pdo->prepare("INSERT INTO movements
                    (product_id, movement_type, movement_quantity, movement_at)
    ...
    public function disable(int $productId): bool {
        $upd = $this->pdo->prepare("UPDATE products
